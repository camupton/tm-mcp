...
const { highlevelTools, executeHighLevelTool } = require('./highlevel-tools');
...
// HighLevel tools endpoint
  app.post('/highlevel/:tool', auth, async (req, res) => {
    const { tool } = req.params;
    const args = req.body;
    
    try {
      const result = await executeHighLevelTool(tool, args);
      res.status(200).json(result);
    } catch (error) {
      console.error(`Error executing HighLevel tool ${tool}:`, error);
      res.status(500).json({
        error: 'Failed to execute HighLevel tool',
        details: error.message
      });
    }
  });

  // Get available HighLevel tools
  app.get('/highlevel/tools', auth, (req, res) => {
    res.status(200).json(highlevelTools);
  });
